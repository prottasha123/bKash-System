...
def load_data(users_file, transactions_file):
    users = {}
    transactions = []
    ...
    if not os.path.exists(users_file):
        ...
        sheet_users.append(["User ID", "Name", "Phone Number"])
        ...
        for row in sheet_users.iter_rows(min_row=2, values_only=True):
            user_id, name, phone_number = row
            user = User(user_id, name, phone_number)
            users[user_id] = user

    # Load or create the transactions Excel file
    if not os.path.exists(transactions_file):
        wb_transactions = openpyxl.Workbook()
        sheet_transactions = wb_transactions.active
        sheet_transactions.append(["Transaction ID", "Sender ID", "Receiver ID", "Amount", "Date"])
        wb_transactions.save(transactions_file)
    else:
        wb_transactions = openpyxl.load_workbook(transactions_file)
        sheet_transactions = wb_transactions.active
        for row in sheet_transactions.iter_rows(min_row=2, values_only=True):
            transaction_id, sender_id, receiver_id, amount, date = row
            sender = users.get(sender_id)
            receiver = users.get(receiver_id)
            if sender and receiver:
                transaction = Transaction(transaction_id, sender, receiver, amount, date)
...
def load_transactions(filename="transactions.xlsx"):
    ...
    try:
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            transaction_id, sender_id, receiver_id, amount, date = row
            transactions.append((transaction_id, sender_id, receiver_id, amount, date))
    except FileNotFoundError:
        print("Transactions file not found. Starting with an empty list.")
    return transactions
